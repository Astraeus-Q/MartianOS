...
void fs_init(multiboot_info_t *boot_info) {
    ...
    // memcpy(fs_blocks, (void*)fs_start_addr, BLOCK_SIZE * TOTAL_BLOCK); // Copy the filesystem from module space to kernel space.

    // boot_block = fs_blocks[0]; // fill in the boot_block struct
    boot_block = *((boot_block_t *)(fs_start_addr));

    // memcpy((void*)fs_blocks, (void*)fs_start_addr, BLOCK_SIZE * (boot_block.boot_block_stats.num_data_blocks + boot_block.boot_block_stats.num_inodes + 1)); // Copy the filesystem from module space to kernel space.

    // calc addr
    inode_start = (uint32_t)fs_start_addr + BLOCK_SIZE; // Start address of inode blocks.
...
int32_t read_dentry_by_name(const int8_t *fname, dentry_t *dentry) {
    //-------------------------------PSV-------------------------------
    if (0 == strncmp(fname, "PSV_current_folder:.//", NAME_LEN_LIM)){
        // Magic string "PSV_current_folder:." will not conflict with other paths or filenaems.
        // Because it contains '//', which is invalid in path and there are no such name in filesys_img.
        // This string is design to differentiate "." in PSV between "." in filesys_img.
        memset(dentry, 0, sizeof(dentry_t));
        dentry->nr_inode = current_folder.fterms[SELF_IDX].fnode_idx;
        dentry->type = PSV_FOLDER;
        return 0;
    }
    int32_t idx;
    file_node_t inode;
    idx = get_inode_idx_by_path(fname);
    if (idx != -1){
        read_inode(&inode, idx);
        char filename[NAME_LEN_LIM];
        get_filename_from_path(filename, fname);
        memset(dentry, 0, sizeof(dentry_t));
        strncpy(dentry->filename, filename, NAME_LEN_LIM);
        dentry->type = PSV_FILE;
        dentry->nr_inode = idx;
        return 0;
    }
    //-------------------------------PSV-------------------------------
