...
int syscall_open(const uint8_t *filename)
{
    ...
    for (idx = 2; idx < 8; idx++)
    {
        if (cur_pcb_ptr->pcb_fds[idx].flags == FREE)
        {
            ...
            //-------------------------------PSV-------------------------------
            case PSV_FOLDER:
                cur_pcb_ptr->pcb_fds[idx].op_ptr = (uint32_t)&PSV_folder_ops;
                break;
            case PSV_FILE:
                cur_pcb_ptr->pcb_fds[idx].op_ptr = (uint32_t)&PSV_file_ops;
                break;
            //-------------------------------PSV-------------------------------
            default:
                break;
...
// ! We always assign the same virtual address (FISH_MAP) to user
// ! but the map to physical address differs
int32_t syscall_vidmap(uint8_t **screen_start)
{
    ...
    if (running_term_ptr->tid != viewing_term_ptr->tid)
        ...
        switch_usrmap(running_term_ptr->vid_mem_buffer >> 22, 1);
    else
        // switch_usrmap(VIDEO >> 12, 1);
        switch_usrmap(VBE_START >> 22, 1);

    flush_tlb();
    // *screen_start = (uint8_t *)(FISH_MAP);
    *screen_start = (uint8_t *)FISH_VIRTUAL;
...
// New syscalls:
/* 
 * Input: Abolute or relative path.
 * Output: 0 for fail and 1 for success.
 * Side Effects: read the value in cr3
 * Reference: OSdev
 */
int32_t syscall_goin(const uint8_t* path){
    return goin(path);
}

int32_t syscall_createfolder(const uint8_t* path){
    return PSV_create_folder(path);
}
int32_t syscall_deletefolder(const uint8_t* path){
    return PSV_delete_folder(path);
}
int32_t syscall_createfile(const uint8_t* path){
    return PSV_create_file(path);
}
int32_t syscall_deletefile(const uint8_t* path){
    return PSV_delete_file(path);
}
